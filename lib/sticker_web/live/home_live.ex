defmodule StickerWeb.HomeLive do
  ...
  @face_sticker_prompt "a cute, clean portrait sticker with a white background"
  ...
  def handle_event("save", %{"prompt" => prompt}, socket) do
    ...
    has_upload? = Enum.any?(socket.assigns.uploads.image.entries, & &1.done?)
    prompt = face_sticker_prompt(prompt, has_upload?)
  ...
  def face_sticker_prompt(prompt, true) do
    prompt
    |> String.trim()
    |> case do
      "" -> @face_sticker_prompt
  ...
  def face_sticker_prompt(prompt, false), do: String.trim(prompt)
